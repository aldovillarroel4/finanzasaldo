...
// --- START: "Anteriores" (last 5 Drive backups) dropdown and restore logic ---
const previousBtn = document.getElementById('previousBackupsBtn');
const previousDropdown = document.getElementById('previousBackupsDropdown');
const previousList = document.getElementById('previousBackupsList');

async function fetchLastDriveBackupsList() {
  try {
    if (!currentGoogleUser && !window.currentGoogleUser) {
      previousList.innerHTML = '<div style="color:#999;padding:8px;text-align:center;">Inicie sesión para ver respaldos</div>';
      return;
    }
    if (!driveAccessToken) {
    ...
    if (!driveAccessToken) {
      previousList.innerHTML = '<div style="color:#999;padding:8px;text-align:center;">Token Drive no disponible</div>';
      return;
    }
    ...
    const url = `https://www.googleapis.com/drive/v3/files?q=${q}&orderBy=createdTime desc&pageSize=5&fields=${fields}`;
    ...
    if (!res.ok) {
      previousList.innerHTML = `<div style="color:#999;padding:8px;text-align:center;">Error listando respaldos: ${res.status}</div>`;
      return;
    }

    const json = await res.json();
    const files = json.files || [];
    populatePreviousBackupsDropdown(files);
  } catch (err) {
    console.error('Error fetching backups list:', err);
    previousList.innerHTML = '<div style="color:#999;padding:8px;text-align:center;">Error cargando respaldos</div>';
  }
}

function populatePreviousBackupsDropdown(files) {
  previousList.innerHTML = '';
  if (!files || files.length === 0) {
    previousList.innerHTML = '<div style="color:#999;padding:8px;text-align:center;">No se encontraron respaldos</div>';
    return;
  }
  ...
  files.forEach(f => {
    ...
    date.textContent = new Date(f.createdTime).toLocaleString();

    const restoreBtn = document.createElement('button');
    restoreBtn.className = 'backup-option';
    restoreBtn.style.marginLeft = '8px';
    restoreBtn.textContent = 'Cargar';
    restoreBtn.onclick = (ev) => {
      ev.stopPropagation();
      restoreDriveBackupById(f.id, f.name);
    };
    ...
    // Allow clicking the row to also restore
    row.addEventListener('click', () => restoreDriveBackupById(f.id, f.name));

    previousList.appendChild(row);
  });
}

async function restoreDriveBackupById(fileId, fileName) {
  try {
    if (!driveAccessToken) {
      await requestDriveToken(true);
    }
    if (!driveAccessToken) {
      alert('No se pudo obtener token de Drive.');
      return;
    }
    ...
    if (!res.ok) {
      ...
      console.error('Error descargando respaldo:', res.status, t);
      alert('Error descargando respaldo desde Drive: ' + res.statusText);
      return;
    }

    const state = await res.json();
    // Use existing restoreAppState to apply loaded backup
    ...
    // close dropdown after restore
    previousDropdown.classList.remove('show');
  } catch (err) {
    console.error('Error restaurando respaldo desde Drive:', err);
    alert('Ocurrió un error al restaurar respaldo: ' + (err.message || err));
  }
}
...
if (previousBtn) {
  previousBtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    // toggle dropdown
    const showing = previousDropdown.classList.toggle('show');
    // if opening, fetch latest list
    if (showing) {
      previousList.innerHTML = '<div style="color:#999;padding:8px;text-align:center;">Cargando...</div>';
      await fetchLastDriveBackupsList();
    }
  });
}

// Close the previous dropdown when clicking outside
document.addEventListener('click', (e) => {
  if (previousDropdown && previousBtn && !previousBtn.contains(e.target) && !previousDropdown.contains(e.target)) {
    previousDropdown.classList.remove('show');
  }
});
// --- END: "Anteriores" dropdown logic ---
...
let currentGoogleUser = null;
 ...
 function onGoogleUserLoggedIn(userData) {
   ...
   // Hide Google sign-in button and show info + logout
   ...
   if (signOutBtn) {
     ...
     signOutBtn.onclick = () => {
       // Clear stored JWT and Drive token, update UI, and disable auto-select if available
