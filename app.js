...
let currentGoogleUser = null;
 ...
 function onGoogleUserLoggedIn(userData) {
   ...
   if (signOutBtn) {
     ...
     signOutBtn.onclick = () => {
       // Clear stored JWT, update UI, and disable auto-select if available
       try { localStorage.removeItem('google_jwt'); } catch(e){/*ignore*/}
       driveAccessToken = null; // in-memory only
   ...
   // Ensure token client initialized for Drive (safe to call repeatedly)
   try {
     initDriveTokenClient();
   } catch (err) {
     console.warn('initDriveTokenClient() error on login:', err);
   }

   // Do NOT automatically load backups from Drive on sign-in.
   // Drive operations will request a fresh token when the user clicks "Guardar" or "Cargar".
 }
...
async function saveBackupToDrive() {
  // MOD DRIVE USAGE: always request fresh token (silent if possible), retry with consent if needed
  try {
    ...
    try {
      // Try to obtain a fresh token silently (promptConsent = false)
      await requestDriveToken(false);
    } catch (errSilent) {
      // If silent request failed due to missing consent/permission, request with consent once
      try {
        await requestDriveToken(true);
      } catch (errConsent) {
        console.error('No se pudo obtener token de Drive tras prompt consent:', errConsent);
        alert('No se pudo obtener permiso para acceder a Google Drive. Revise permisos o intente iniciar sesión nuevamente.');
        return;
      }
    }
    ...
    try {
      ...
      console.error('Network error uploading to Drive:', networkErr);
      alert('Error de red al subir respaldo a Google Drive.');
      return;
    }

    if (!res.ok) {
      // If unauthorized, try to re-request consent and retry once
      if (res.status === 401 || res.status === 403) {
        try {
          await requestDriveToken(true);
          const retryRes = await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,parents,createdTime', {
            method: 'POST',
          ...
          if (!retryRes.ok) {
            ...
            console.error('Retry error al subir backup a Drive:', retryRes.status, text);
            alert('Error al guardar respaldo en Google Drive: ' + retryRes.statusText);
            return;
          }
          const resultRetry = await retryRes.json();
          // Restore and inform user
          restoreAppState(appState);
          try {
            localStorage.setItem('last_backup_name', resultRetry.name);
            const el = document.getElementById('lastBackupName');
            if (el) el.textContent = resultRetry.name;
          } catch (e) { /* ignore */ }
          saveToLocalStorage();
          saveSalonToLocalStorage();
          alert(`Respaldo guardado y cargado correctamente como "${resultRetry.name}"`);
          return;
        } catch (consErr) {
          console.error('Error re-obteniendo token con consentimiento para retry:', consErr);
          alert('No se pudo obtener permiso para subir a Google Drive.');
          return;
        }
      }
    ...
    try {
      ...
      // Persist and show the filename of the restored backup
      try {
        localStorage.setItem('last_backup_name', result.name);
        const el = document.getElementById('lastBackupName');
